...
from Project.models.user_project import (
    CommentReport,
    ProjectComment,
    ProjectDonation,
    ProjectRate,
    ...
    if request.method == "GET":
        ...
        return render(
            ...
            "project/show.html",
            {
                ...
                "project": project,
                "rate": rate,
                "yellowRate": range(1, int(rate) + 1),
                "blackRate": range(int(rate + 1), 6),
                "relatedPorjects": projectDic,
                "pictures": projectPics,
            },
        )
...
def comment(request):
    ...
    return redirect(f'/project/{request.POST.get("project_id")}')
...
def report(request):
    ...
    report = ProjectReport.objects.create(
        report_body=request.POST.get("report_body"),
        user_id=request.POST.get("user_id"),
        project_id=request.POST.get("project_id"),
    )
    return redirect(f'/project/{request.POST.get("project_id")}')
...
def rate(request):
    ...
    if request.is_ajax():
        previous_rate_count = ProjectRate.objects.filter(
            project_id=request.GET.get("proectId"), user_id=request.GET.get("userId")
        ).count()
        if previous_rate_count == 0:
            ...
            previous_rate_count = ProjectRate.objects.filter(
                project_id=request.GET.get("proectId"),
                user_id=request.GET.get("userId"),
            ).update(rate=float(request.GET.get("rate")))
...
def donate(request):
    ...
    if request.is_ajax():
        ...
        ).filter(id=request.GET.get("proectId"))
        ...
        if int(total_target[0]) - int(total_donates) < int(request.GET.get("donate")):
            return HttpResponse(
                "project doesnot need this amount you can only donate with %s"
                % str(int(total_target[0]) - int(total_donates))
            )
        elif int(userBalance[0]) >= int(request.GET.get("donate")):
            dona = ProjectDonation.objects.create(
                amount=float(request.GET.get("donate")),
                user_id=request.GET.get("userId"),
                project_id=request.GET.get("proectId"),
            )
            User.objects.filter(id=request.GET.get("userId")).update(
                balance=int(userBalance[0]) - int(request.GET.get("donate"))
            )
